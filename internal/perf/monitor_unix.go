...
func tryIntelGpuTop(ctx context.Context, every time.Duration, logger *logmon.Monitor) (chan []GpuStat, error) {
	...
	go func() {
		...
		// Unlike mactop (one JSON object per line, see tryMactop), intel_gpu_top -J
		// pretty-prints objects across many lines inside an outer "[" that never
		// closes, with objects that may or may not be comma-separated. stdlib json
		// rejects both variants, so accumulate each top-level object by brace depth.
